...
Ya establecida la forma en que se buscarán los préstamos nuevos,  el procedimiento para la presentación de resultados es el siguiente. 
...
\item Se proporcionará en \cite{prestamos_nuevos} las listas  de préstamos nuevos entre cada pareja de idioma origen e idioma receptor, agrupados por el año de aparición.  Se especificará en \ref{lectura.listas}  la forma de leer las listas.

\item Por cada resultado y por cada par de idiomas, se agregará un contexto de los sucesos relacionados a las migraciones. Esto permite omitir los resultados del punto 2 (ver  página \pageref{proceso.nuevos}), ya que muestra los campos donde \textit{A} influenció a  \textit{B}, y  de manera implícita los campos donde \textit{B} influenció a \textit{A}.
...
Para las gráficas se utilizaron diferentes colores  y un sistema de abreviaciones para distinguir a los idiomas que intervienen, correspondiendo la primera abreviación al origen y la segunda al receptor. Los colores y abreviaciones de cada idioma se especifican  en la tabla \ref{tab.idcolor}. 

En todas las gráficas, el eje horizontal corresponde a los años del conjunto de búsqueda (1900-2009),  mientras que en el eje vertical representa la cantidad de prestamos nuevos $N_{p}$. 
