...
interpretar la influencia de un idioma sobre otro. Estas son: 

\begin{enumerate}
	\label{proceso.nuevos}
	
\item Contando por cada año los préstamos nuevos de origen \textit{A} que están presentes en los diferentes receptores. Esto muestra en cuales idiomas \textit{A} ha sido influyente. 

\item Contando cuántos préstamos nuevos de diferentes orígenes están presentes en cada año, si se toma a \textit{A} como receptor. Con ello se obtienen los idiomas que han influenciado a \textit{A}
...
\cpnote{Josue, por favor revisa el 
uso de comas y punto y coma. Creo que no lo estas usando bien. También trata de ser consistente con los tiempos de los verbos. Nota que en el punto paso usas ``Contando'', luego ahora usas ``Tomar'' y luego ``Contando''. No es consistente. Porfa corrige  este error y propaga. Dale una iterada a este capitulo.}
\jmnote{listo}

\item Tomando fijos un idioma \textit{A} y un idioma \textit{B}, contando cuántos préstamos nuevos de \textit{A} están en \textit{B} y cuantos de \textit{B} lo están en \textit{A}.  Así se obtiene cómo ha sido la influencia entre \textit{A} y \textit{B}.
...
\cpnote{Tambien mejora la redaccion de esto. Usa puntos para separar las
ideas. Perdon estar molestando tanto, pero de verdad esta dificil de leer y la tesis no debe quedar asi.}
\jmnote{listo, ya corregi la redaccion, quedo mejor}
...
que originan las migraciones. Por ejemplo, sucesos como la globalización y el acceso al desarrollo tecnológico a finales de 1980 y principios de 1990, propició la migración  de términos como \textit{internet}, \textit{computadora}, \textit{web}, \textit{email} o \textit{software}, siendo el movimiento de este grupo de palabras, una consecuencia del desarrollo e impacto de ambos sucesos. 
 
\cpnote{cuales ámbitos?} \jmnote{cambie la palabra ambito por suceso}

\cpnote{Como se propaga un movimiento? Creo que intetas adornar de mas las frases y no  qeuda bien.}
\jmnote{ok, queria dar a entender que el movimiento era una consecuencia de que se desarrollaran tales ambitos}

Las causas que originan las migraciones, serán identificables a partir del significado de los préstamos involucrados en un año de migración. Si un \textbf{campo semántico} lo forman un conjunto de palabras que están relacionadas por su significado, entonces para un evento ocurrido en el mismo año (o en los años al rededor de él), las palabras migrantes pueden pertenecer a un mismo campo semántico relacionado al evento.
\cpnote{Que es un campo semántico?} 

\cpnote{Porfa no separes las correcciones que hago del parrafo o la frase en la
que la hago pues luego ya no se a que se refiere}
...
Descritas las características de los préstamos nuevos, la obtención y presentación de resultados se realizó de la siguiente manera. 


%Descrita la forma en que se buscarán los préstamos nuevos,  el procedimiento para la obtención y  la presentación de resultados es el siguiente. 
...
En las gráficas se utilizaron diferentes colores y un sistema de abreviaciones para distinguir a los idiomas que intervienen, correspondiendo la primera abreviación al idioma origen y la segunda al idioma receptor. Los colores y abreviaciones se especifican  en la tabla \ref{tab.idcolor}. 

En todas las gráficas, el eje horizontal corresponde a los años del conjunto de búsqueda (1900-2009),  mientras el eje vertical representa la cantidad de prestamos nuevos $N_{p}$. 
...
\begin{figure}[h!]
	...
	\label{fig.NC_FR}
	\caption{Palabras nuevas del francés hacia los demás idiomas. Entre las demás lenguas romances, el aporte del francés ha sido minoritario siendo las lenguas germánicas donde el francés aportó más elementos, principalmente en el alemán en las décadas durante la primera (1920) y segunda guerra mundial (1940).}
...
Durante el siglo XX, los préstamos en este sentido, se caracterizan por ser palabras comunes e identificables de origen inglés,  por ejemplo  \textit{diagnostic,} \textit{clients,} \textit{placement,} \textit{adaptation,} \textit{diffusion,} \textit{amplitude,}.  A pesar de ser errores en los resultados, este tipo de palabras si surgieron primero en el francés, por lo que el algoritmo designó a esta idioma como el origen. 
...
Las clasificaciones para esta pareja de origen y receptor son escasas, entre las pocas realizadas se encuentra un término del campo científico como \textit{Poincaré} (1924) (apellido del matemático francés Henri Poincaré);  y nombres de ciudades y países referentes tras las dos guerras mundiales, \textit{Versailles} (1924), \textit{Vietnam} (1966)  y \textit{URSS} (1975).
...
\begin{figure}[h!]
	...
	\label{fig.NC_GE}
	\caption{Palabras nuevas del alemán hacia los demás idiomas. El francés destaca  entre 1930-1940 por ser el idioma con más préstamos del alemán. Se distingue al español por tener periodos donde no recibió  ninguna palabra.}  
\end{figure}
...
El primer préstamo con contenido es \textit{Panamá} (1913), su importancia se debe a la inauguración del canal de Panamá en 1914, destaca que la palabra llegue al francés por ser el gobierno de Francia el que impulsó económicamente su construcción, aunque su conclusión y administración pasó a los Estados Unidos.  
...
Las áreas mencionadas, no brindan una respuesta sobre que idioma ha sido más influyente, sino en cuales campos un idioma ha influido más que los otros, siendo esta una forma alternativa de hablar de influencia en los idiomas.
